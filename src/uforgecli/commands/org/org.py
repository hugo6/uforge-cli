...
class Org_Cmd(Cmd, CoreGlobal):
        """Organization administration (list/info/create/update/delete etc)"""
        ...
        def do_info(self, args):
                try:
                        ...
                        if orgInfo.galleryUri is not None:
                                ...
                                galleryUri = "No"
                        table.add_row(["Default Org", galleryUri])
                        print table.draw() + "\n"
        ...
        def do_create(self, args):
                try:
                        ...
                        printer.out("Creating object...")
                        orgCreated = org()
                        orgCreated.name = doArgs.org
                        if doArgs.autoActivate:
                                autoActivate = "true"
                        else:
                                autoActivate = "false"
                        if doArgs.hasStore:
                                ...
                                hasStore = "false"
                        self.api.Orgs.Create(Autoactivate=autoActivate, Hasgallery=hasStore, body=orgCreated)
                        printer.out("Org ["+orgCreated.name+"] has been correctly created.", printer.OK)
